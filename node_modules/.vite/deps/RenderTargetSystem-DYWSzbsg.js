import { G as Texture, H as warn, J as TextureSource, K as TextureMatrix, N as DOMAdapter, R as BigPool, U as Color, V as GlobalResourceRegistry, W as Bounds, d as BindGroup, f as UniformGroup, g as GpuProgram, gt as extensions, ht as ExtensionType, lt as Matrix, mt as eventemitter3_default, nt as v8_0_0, ot as uid$1, rt as v8_21_0, st as Rectangle, t as CanvasPool, tt as deprecation, u as RendererType, ut as Point, w as GlProgram } from "./CanvasPool-DOie_UcP.js";
import { D as multiplyColors, L as FilterEffect, O as getGlobalBounds, _ as TexturePool, c as Sprite, g as RenderGroup, i as Ticker, r as State, s as CanvasSource, t as Filter, u as Container, w as getLocalBounds } from "./Filter-oI80x9ov.js";
import { F as STENCIL_MODES, n as DefaultBatcher } from "./GCManagedHash-Ct0R8-Q-.js";
...
var _BatcherPipe = class _BatcherPipe {
	...
	*/
	destroyInstructionSet(instructionSet) {
		const batchers = this._batchersByInstructionSet[instructionSet.uid];
		if (!batchers) return;
		for (const i in batchers) batchers[i].destroy();
...
var StencilMaskPipe = class {
	...
	execute(instruction) {
		const gpuRenderer = this._renderer;
...
var _GCSystem = class _GCSystem {
	...
	*/
	removeResource(resource) {
		...
		if (this._running) this._managedResources[index] = null;
		else {
			if (index !== last) {
	...
	*/
	...
	runOnResource(resource, now) {
		if (resource._gcData.type === "renderable") this.updateRenderableGCTick(resource, now);
		if (now - resource._gcLastUsed < this.maxUnusedTime || !resource.autoGarbageCollect) return;
...
var _RenderTarget = class _RenderTarget extends eventemitter3_default {
	/**
...
var RenderTargetSystem = class {
	...
	*/
	isFrontFaceInverted(renderTarget, flipY) {
		if (!renderTarget) {
	...
	*/
	...
	destroy() {
		...
		this._renderSurfaceToRenderTargetHash.forEach((renderTarget, key) => {
			...
			else renderTarget.off("destroy", this._onRenderTargetDestroy, this);
		});
		this._renderSurfaceToRenderTargetHash.clear();
		for (const gpuRenderTarget of Object.values(this._gpuRenderTargetHash)) if (gpuRenderTarget) this.adaptor.destroyGpuRenderTarget(gpuRenderTarget);
		this._gpuRenderTargetHash = /* @__PURE__ */ Object.create(null);
	}
	...
	*/
	_onRenderTargetDestroy(renderTarget) {
	...
	*/
	_releaseGpuRenderTarget(renderTarget) {
